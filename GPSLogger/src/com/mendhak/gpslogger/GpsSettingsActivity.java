...
public class GpsSettingsActivity extends PreferenceActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (!Utilities.Flag()) {
			...
			seemymapSetup
					.setSummary("Available in the Pro version, coming soon");
		...
		if (useImperial) {
			...
			distanceBeforeLogging.getEditText()
					.setHint("Enter feet (max 9999)");
		} else {
			distanceBeforeLogging.setDialogTitle("Distance in meters");
			distanceBeforeLogging.getEditText().setHint(
					"Enter meters (max 9999)");
		}
					...
					public boolean onPreferenceChange(Preference preference,
						...
						new Thread() {
							...
							public void run() {
								...
								boolean useImp = new Boolean(newValue
										.toString());
								...
								if (minimumDistanceString != null
										&& minimumDistanceString.length() > 0) {
								...
								if (useImp == true) {
									distanceBeforeLogging
											.setDialogTitle("Distance in feet");
									distanceBeforeLogging.getEditText()
											.setHint("Enter feet (max 9999)");

									minimumDistance = Utilities
											.MetersToFeet(minimumDistance);

								} else {
									minimumDistance = Utilities
											.FeetToMeters(minimumDistance);
									distanceBeforeLogging
											.setDialogTitle("Distance in meters");
									distanceBeforeLogging.getEditText()
											.setHint("Enter meters (max 9999)");
								...
								editor.putString("distance_before_logging",
